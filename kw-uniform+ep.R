...
for (l in 1:K) {
  ...
  for (t_ind in 1:length(p)) {
    ...
    for (i in 1:n) {
      Q[1,1]<-Q[1,1]+colSums(ker_x2[[i]]%*%t(t(local_kernel2(S.z[[i]]-t,h2_kw))))/n
      Q[1,2]<-Q[1,2]+colSums(ker_x2t1[[i]]%*%t(t(local_kernel2(S.z[[i]]-t,h2_kw))))/n
      Q[1,3]<-Q[1,3]+colSums(ker_x1[[i]]%*%t(t(local_kernel2(S.z[[i]]-t,h2_kw)*z[[i]])))/n
      Q[1,4]<-Q[1,4]+colSums(ker_x1[[i]]%*%t(t(local_kernel2(S.z[[i]]-t,h2_kw)*z[[i]]*(S.z[[i]]-t))))/n
      Q[2,2]<-Q[2,2]+colSums(ker_x2t2[[i]]%*%t(t(local_kernel2(S.z[[i]]-t,h2_kw))))/n
      Q[2,3]<-Q[2,3]+colSums(ker_x1t1[[i]]%*%t(t(local_kernel2(S.z[[i]]-t,h2_kw)*z[[i]])))/n
      Q[2,4]<-Q[2,4]+colSums(ker_x1t1[[i]]%*%t(t(local_kernel2(S.z[[i]]-t,h2_kw)*z[[i]]*(S.z[[i]]-t))))/n
      Q[3,3]<-Q[3,3]+colSums(ker[[i]]%*%t(t(local_kernel2(S.z[[i]]-t,h2_kw)*z[[i]]*z[[i]])))/n
      Q[3,4]<-Q[3,4]+colSums(ker[[i]]%*%t(t(local_kernel2(S.z[[i]]-t,h2_kw)*z[[i]]*z[[i]]*(S.z[[i]]-t))))/n
      Q[4,4]<-Q[4,4]+colSums(ker[[i]]%*%t(t(local_kernel2(S.z[[i]]-t,h2_kw)*z[[i]]*z[[i]]*(S.z[[i]]-t)*(S.z[[i]]-t))))/n
      
      q[1,]<-q[1,]+colSums(ker_xy[[i]]%*%t(t(local_kernel2(S.z[[i]]-t,h2_kw))))/n
      q[2,]<-q[2,]+colSums(ker_xyt[[i]]%*%t(t(local_kernel2(S.z[[i]]-t,h2_kw))))/n
      q[3,]<-q[3,]+colSums(ker_y[[i]]%*%t(t(local_kernel2(S.z[[i]]-t,h2_kw)*z[[i]])))/n
      q[4,]<-q[4,]+colSums(ker_y[[i]]%*%t(t(local_kernel2(S.z[[i]]-t,h2_kw)*z[[i]]*(S.z[[i]]-t))))/n
    ...
    for (i in 1:n) {
      ...
      p11<-t(as.vector(local_kernel(T.x[[i]]-t,h1_kw)*x[[i]])%o%as.vector(local_kernel2(S.z[[i]]-t,h2_kw)))%o%(matrix(rep(y[[i]]-x[[i]]*(betagamma.hat[1,1]+betagamma.hat[2,1]*(T.x[[i]]-t)),nz[i]),ncol = nz[i],nrow = nx[i])-matrix(rep(z[[i]]*(betagamma.hat[3,1]+betagamma.hat[4,1]*(S.z[[i]]-t)),nx[i]),ncol = nz[i],nrow = nx[i],byrow = T))
      p1<-matrix(0, ncol=nx[i], nrow=nz[i])
      p22<-array(0,c(nz[i],nx[i],nx[i],nz[i]))
      p22<-t(as.vector(local_kernel(T.x[[i]]-t,h1_kw)*(T.x[[i]]-t)*x[[i]])%o%as.vector(local_kernel2(S.z[[i]]-t,h2_kw)))%o%(matrix(rep(y[[i]]-x[[i]]*(betagamma.hat[1,1]+betagamma.hat[2,1]*(T.x[[i]]-t)),nz[i]),ncol = nz[i],nrow = nx[i])-matrix(rep(z[[i]]*(betagamma.hat[3,1]+betagamma.hat[4,1]*(S.z[[i]]-t)),nx[i]),ncol = nz[i],nrow = nx[i],byrow = T))
      p2<-matrix(0, ncol=nx[i], nrow=nz[i])
      p33<-array(0,c(nz[i],nx[i],nx[i],nz[i]))
      p33<-t(as.vector(local_kernel(T.x[[i]]-t,h1_kw))%o%as.vector(local_kernel2(S.z[[i]]-t,h2_kw)*z[[i]]))%o%(matrix(rep(y[[i]]-x[[i]]*(betagamma.hat[1,1]+betagamma.hat[2,1]*(T.x[[i]]-t)),nz[i]),ncol = nz[i],nrow = nx[i])-matrix(rep(z[[i]]*(betagamma.hat[3,1]+betagamma.hat[4,1]*(S.z[[i]]-t)),nx[i]),ncol = nz[i],nrow = nx[i],byrow = T))
      p3<-matrix(0, ncol=nx[i], nrow=nz[i])
      p44<-array(0,c(nz[i],nx[i],nx[i],nz[i]))
      p44<-t(as.vector(local_kernel(T.x[[i]]-t,h1_kw))%o%as.vector(local_kernel2(S.z[[i]]-t,h2_kw)*(S.z[[i]]-t)*z[[i]]))%o%(matrix(rep(y[[i]]-x[[i]]*(betagamma.hat[1,1]+betagamma.hat[2,1]*(T.x[[i]]-t)),nz[i]),ncol = nz[i],nrow = nx[i])-matrix(rep(z[[i]]*(betagamma.hat[3,1]+betagamma.hat[4,1]*(S.z[[i]]-t)),nx[i]),ncol = nz[i],nrow = nx[i],byrow = T))
