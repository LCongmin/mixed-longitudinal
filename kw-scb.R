...
#MB
K_simu<-10000
abs_G_beta_kw_N01<-rep(0,K_simu)
abs_G_gamma_kw_N01<-rep(0,K_simu)
abs_G_beta_kw_E1<-rep(0,K_simu)
abs_G_gamma_kw_E1<-rep(0,K_simu)
abs_G_beta_kw_P<-rep(0,K_simu)
abs_G_gamma_kw_P<-rep(0,K_simu)
abs_G_beta_kw_R<-rep(0,K_simu)
abs_G_gamma_kw_R<-rep(0,K_simu)
abs_G_beta_kw_NN<-rep(0,K_simu)
abs_G_gamma_kw_NN<-rep(0,K_simu)
abs_G_beta_kw_22<-rep(0,K_simu)
abs_G_gamma_kw_22<-rep(0,K_simu)


betagamma<-array(0,c(4,1,length(p)))
hatvar<-array(0,c(4,4,length(p)))
G_kw_beta_N01<-matrix(0,ncol = length(p),nrow = K_simu)
G_kw_gamma_N01<-matrix(0,ncol = length(p),nrow = K_simu)
G_kw_beta_E1<-matrix(0,ncol = length(p),nrow = K_simu)
G_kw_gamma_E1<-matrix(0,ncol = length(p),nrow = K_simu)
G_kw_beta_P<-matrix(0,ncol = length(p),nrow = K_simu)
G_kw_gamma_P<-matrix(0,ncol = length(p),nrow = K_simu)
G_kw_beta_R<-matrix(0,ncol = length(p),nrow = K_simu)
G_kw_gamma_R<-matrix(0,ncol = length(p),nrow = K_simu)
G_kw_beta_NN<-matrix(0,ncol = length(p),nrow = K_simu)
G_kw_gamma_NN<-matrix(0,ncol = length(p),nrow = K_simu)
G_kw_beta_22<-matrix(0,ncol = length(p),nrow = K_simu)
G_kw_gamma_22<-matrix(0,ncol = length(p),nrow = K_simu)
...
for (t_ind in 1:length(p)) {
  ...
  for (k_simu in 1:K_simu) {
    #for (i in 1:n) {
    tau_N01<-rnorm(n,0,1)
    G_kw_beta_N01[k_simu,t_ind]<-ginv(Q[1,1])*sum(tau_N01*p1_1)/n
    G_kw_gamma_N01[k_simu,t_ind]<-ginv(Q[3,3])*sum(tau_N01*p3_1)/n
    
    tau_E1<-rexp(n,1)-1
    G_kw_beta_E1[k_simu,t_ind]<-ginv(Q[1,1])*sum(tau_E1*p1_1)/n
    G_kw_gamma_E1[k_simu,t_ind]<-ginv(Q[3,3])*sum(tau_E1*p3_1)/n
    
    tau_P<-rpois(n,1)-1
    G_kw_beta_P[k_simu,t_ind]<-ginv(Q[1,1])*sum(tau_P*p1_1)/n
    G_kw_gamma_P[k_simu,t_ind]<-ginv(Q[3,3])*sum(tau_P*p3_1)/n
    
    tau_B<-rbinom(n,1,0.5)
    tau_B[which(tau_B==0)]=-1
    G_kw_beta_R[k_simu,t_ind]<-ginv(Q[1,1])*sum(tau_B*p1_1)/n
    G_kw_gamma_R[k_simu,t_ind]<-ginv(Q[3,3])*sum(tau_B*p3_1)/n
    ...
    G_kw_beta_NN[k_simu,t_ind]<-ginv(Q[1,1])*sum(tau_NN*p1_1)/n
    G_kw_gamma_NN[k_simu,t_ind]<-ginv(Q[3,3])*sum(tau_NN*p3_1)/n
    
    tau22=rbinom(n,1,(sqrt(5)+1)/(2*sqrt(5)))
    tau22[which(tau22==1)]=-(sqrt(5)-1)/2
    tau22[which(tau22==0)]=(sqrt(5)+1)/2
    G_kw_beta_22[k_simu,t_ind]<-ginv(Q[1,1])*sum(tau22*p1_1)/n
    G_kw_gamma_22[k_simu,t_ind]<-ginv(Q[3,3])*sum(tau22*p3_1)/n
...
for (k_simu in 1:K_simu) {
  abs_G_beta_kw_N01[k_simu]<-max(abs(G_kw_beta_N01[k_simu,]))
  abs_G_gamma_kw_N01[k_simu]<-max(abs(G_kw_gamma_N01[k_simu,]))
  
  abs_G_beta_kw_E1[k_simu]<-max(abs(G_kw_beta_E1[k_simu,]))
  abs_G_gamma_kw_E1[k_simu]<-max(abs(G_kw_gamma_E1[k_simu,]))
  
  abs_G_beta_kw_P[k_simu]<-max(abs(G_kw_beta_P[k_simu,]))
  abs_G_gamma_kw_P[k_simu]<-max(abs(G_kw_gamma_P[k_simu,]))
  
  abs_G_beta_kw_R[k_simu]<-max(abs(G_kw_beta_R[k_simu,]))
  abs_G_gamma_kw_R[k_simu]<-max(abs(G_kw_gamma_R[k_simu,]))
  
  abs_G_beta_kw_22[k_simu]<-max(abs(G_kw_beta_22[k_simu,]))
  abs_G_gamma_kw_22[k_simu]<-max(abs(G_kw_gamma_22[k_simu,]))
  
  abs_G_beta_kw_NN[k_simu]<-max(abs(G_kw_beta_NN[k_simu,]))
  abs_G_gamma_kw_NN[k_simu]<-max(abs(G_kw_gamma_NN[k_simu,]))
}

quantile(abs_G_beta_kw_R,0.95);quantile(abs_G_gamma_kw_R,0.95)
quantile(abs_G_beta_kw_22,0.95);quantile(abs_G_gamma_kw_22,0.95)
quantile(abs_G_beta_kw_N01,0.95);quantile(abs_G_gamma_kw_N01,0.95)
quantile(abs_G_beta_kw_P,0.95);quantile(abs_G_gamma_kw_P,0.95)
quantile(abs_G_beta_kw_NN,0.95);quantile(abs_G_gamma_kw_NN,0.95)
quantile(abs_G_beta_kw_E1,0.95);quantile(abs_G_gamma_kw_E1,0.95)
...
plot(density(abs_G_beta_kw_N01),xlim=c(0,2.5),ylim=c(0,6), main = "",xlab = "Values")
#lines(density(abs_G_beta_center_N11),col="red")
lines(density(abs_G_beta_kw_R),col="green",lty=1)
lines(density(abs_G_beta_kw_22),col="gray",lty=1)
lines(density(abs_G_beta_kw_P),col="red",lty=1)
lines(density(abs_G_beta_kw_NN),col="purple",lty=1)
lines(density(abs_G_beta_kw_E1),col="blue")
...
plot(density(abs_G_gamma_kw_N01),xlim=c(0,2.5),ylim=c(0,6),main = "",xlab = "Values")
#lines(density(abs_G_gamma_center_N11),col="red")
lines(density(abs_G_gamma_kw_R),col="green",lty=1)
lines(density(abs_G_gamma_kw_22),col="gray",lty=1)
lines(density(abs_G_gamma_kw_P),col="red",lty=1)
lines(density(abs_G_gamma_kw_NN),col="purple",lty=1)
lines(density(abs_G_gamma_kw_E1),col="blue")
